...
const WA_MSG = encodeURIComponent(
  'Olá, vim pela página de tráfego pago em Mogi das Cruzes e quero uma análise das minhas campanhas.'
)

export function LpTrafegoMogiForm() {
  const [form, setForm] = useState({ nome: '', email: '', whatsapp: '', investimento: '', whatsappOptin: false })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true)
    try {
      const token = await getRecaptchaToken('lp_trafego_mogi')
      const res = await fetch('/api/contact', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          source: 'lp_trafego_mogi',
          _pageUrl: typeof window !== 'undefined' ? window.location.href : '',
          _recaptcha: token,
          Nome: form.nome,
          'E-mail': form.email,
          WhatsApp: form.whatsapp,
          'Investimento mensal em mídia': form.investimento,
    ...
    return (
      <div className="flex flex-col gap-5 py-8 items-center text-center">
        ...
        <p className="text-sm text-zinc-400 leading-relaxed max-w-xs">
          Nosso time de mídia entra em contato em até 1 hora útil para analisar suas campanhas.
        </p>
        <a
  ...
  return (
    <form id="form-trafego-mogi" onSubmit={handleSubmit} className="flex flex-col gap-3">
      <input
        type="text"
        required
      ...
      <select
        ...
        className={inputCls}
      >
        <option value="">Investimento mensal em mídia *</option>
        <option value="Ainda não invisto">Ainda não invisto</option>
        <option value="Até R$ 3 mil">Até R$ 3 mil</option>
        <option value="De R$ 3 mil a R$ 10 mil">De R$ 3 mil a R$ 10 mil</option>
      ...
      >
        {loading ? 'Enviando...' : 'Quero analisar minhas campanhas'}
      </button>
