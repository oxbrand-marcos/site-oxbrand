...
const staticPages: MetadataRoute.Sitemap = [
  ...
  { url: `${BASE}/trafego-pago-mogi-das-cruzes`, lastModified: new Date(), changeFrequency: 'monthly', priority: 0.9 },
  ...
  ...PROFILE_AUTHORS.filter((a) => !['joane-guimaraes', 'maria-clara', 'beatriz-soares', 'gabriel-figueiredo', 'oxbrand'].includes(a.slug)).map((a) => ({ url: `${BASE}/blog/autor/${a.slug}`, lastModified: new Date(), changeFrequency: 'monthly' as const, priority: 0.5 })),
...
const staticPosts: MetadataRoute.Sitemap = [
  ...
  ...CASES.filter((c) => c.published).map((c) => ({
    url: `${BASE}/nossos-clientes/${c.slug}`,
